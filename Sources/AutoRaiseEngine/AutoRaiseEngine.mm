...
static CGEventFlags disableKey = 0;
...
NSArray<NSString *> * splitAndTrimCSV(NSString * value) {
    ...
    for (NSString * item in [value componentsSeparatedByString: @","]) {
        NSString * trimmed = [item stringByTrimmingCharactersInSet: whitespace];
...
void onTick() {
    ...
    CGPoint previousPoint = oldPoint;
    CGFloat mouse_x_diff = mousePoint.x-previousPoint.x;
    CGFloat mouse_y_diff = mousePoint.y-previousPoint.y;
    oldPoint = mousePoint;
...
//------------------------------------------ Quiver engine wrapper -----------------------------------------

@implementation AutoRaiseConfig
@end

@implementation AutoRaiseEngine

+ (BOOL)focusFirstAvailable {
#ifdef FOCUS_FIRST
    return YES;
#else
    return NO;
...
+ (BOOL)isAccessibilityTrusted { return isAccessibilityTrusted(false); }
+ (BOOL)promptAccessibility { return isAccessibilityTrusted(true); }
...
- (BOOL)isRunning { return engineActive; }

- (void)startWithConfig:(AutoRaiseConfig *)config {
    if (engineActive) { return; }

    // Map configuration onto the engine globals (mirrors AutoRaise main()).
    delayCount         = (int) config.delay;
    warpX              = config.warpX;
    warpY              = config.warpY;
    cursorScale        = config.scale < 1 ? 2.0f : config.scale;
    verbose            = config.verbose;
    altTaskSwitcher    = config.altTaskSwitcher;
    mouseDelta         = config.mouseDelta < 0 ? 0 : config.mouseDelta;
    pollMillis         = (int) (config.pollMillis < 20 ? 50 : config.pollMillis);
    ...
    warpMouse = config.warpEnabled &&
        warpX >= 0 && warpX <= 1 && warpY >= 0 && warpY <= 1;

#ifdef FOCUS_FIRST
    if (config.focusDelay > 0) {
        raiseDelayCount = delayCount;
        delayCount = (int) config.focusDelay;
    } else {
        raiseDelayCount = 1;
    }
    if (!delayCount && !raiseDelayCount) { delayCount = 1; }
#else
    if (!delayCount) { delayCount = 1; }
#endif

    NSMutableArray * ignoreA = config.ignoreApps.length
        ? [splitAndTrimCSV(config.ignoreApps) mutableCopy]
        : [[NSMutableArray alloc] init];
    [ignoreA addObject: AssistiveControl];
    ignoreApps = [ignoreA copy];

    ignoreTitles = config.ignoreTitles.length ? splitAndTrimCSV(config.ignoreTitles) : @[];
    stayFocusedBundleIds = config.stayFocusedBundleIds.length
        ? splitAndTrimCSV(config.stayFocusedBundleIds) : @[];

    if ([config.disableKey isEqualToString: @"control"]) {
        disableKey = kCGEventFlagMaskControl;
    } else if ([config.disableKey isEqualToString: @"option"]) {
    ...
    if (eventTap) {
        ...
        if (eventTapRunLoopSource) {
            CFRunLoopAddSource(CFRunLoopGetCurrent(), eventTapRunLoopSource, kCFRunLoopCommonModes);
            CGEventTapEnable(eventTap, true);
        }
    }
...
- (void)stop {
    ...
    if (workspaceWatcher) {
        [NSObject cancelPreviousPerformRequestsWithTarget: workspaceWatcher];
    ...
    if (eventTap) {
        ...
        if (eventTapRunLoopSource) {
            CFRunLoopRemoveSource(CFRunLoopGetCurrent(), eventTapRunLoopSource, kCFRunLoopCommonModes);
    ...
    if (axObserver) {
        CFRunLoopRemoveSource(CFRunLoopGetCurrent(),
            AXObserverGetRunLoopSource(axObserver), kCFRunLoopCommonModes);
        CFRelease(axObserver);
        axObserver = NULL;
    }

    if (_previousFinderWindow) { CFRelease(_previousFinderWindow); _previousFinderWindow = NULL; }
    if (_dock_app) { CFRelease(_dock_app); _dock_app = NULL; }
#ifdef FOCUS_FIRST
    if (_lastFocusedWindow) { CFRelease(_lastFocusedWindow); _lastFocusedWindow = NULL; }
#endif
